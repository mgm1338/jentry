...
import core.stub.*;
import store.col.storage.generic.ColStorage_ValueTypeName_;
 ...
 * </p>
 */
public class ColStorageBlocked_ValueTypeName_ implements ColStorage_ValueTypeName_
{
     ...
     */
    public ColStorageBlocked_ValueTypeName_( int blockSize, int storeSize )
    {
     ...
     */
    public ColStorageBlocked_ValueTypeName_( int blockSize, int size, GrowthStrategy growthStrategy )
    {
        ...
        data = new _val_[ numBlocks ][];
        for( int i = 0; i < numBlocks; i++ )
        {
            data[ i ] = new _val_[ this.blockSize ];
        }
    ...
    @Override
    @UncheckedArray
    public void setValue( _val_ value, int idx )
    {
    ...
    public void grow( int minSize )
    {
        ...
        if( newSize == size ) throw new ArrayGrowthException( this.getClass(), size, minSize, Types._ValueTypeName_ );

        int newNumBlocks = newSize / blockSize;
        if( newSize % blockSize != 0 ) newNumBlocks++;
        _val_[][] temp = new _val_[ newNumBlocks ][ blockSize ];
        for( int i = 0; i < newNumBlocks; i++ )
        {
            temp[ i ] = ( i < this.numBlocks ) ? data[ i ] : new _val_[ blockSize ]; //update or allocate new blocks
        }
     ...
     */
    @UncheckedArray
    public void fill( _val_ val, int fromIndex, int toIndex )
    {
     ...
     */
    @Override
    public ColStorageBlocked_ValueTypeName_ getCopy()
    {
        ColStorageBlocked_ValueTypeName_ copy = new ColStorageBlocked_ValueTypeName_( this.blockSize, this.getCapacity(),
                                                                                  this.growthStrategy );
    ...
    @UncheckedArray
    @Override
    public void copyFrom( ColStorage_ValueTypeName_ source, int srcPos, int destPos, int length )
    {
