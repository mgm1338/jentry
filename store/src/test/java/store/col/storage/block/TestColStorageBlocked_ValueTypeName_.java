 ...
 * Created: 5/7/13
 */
public class TestColStorageBlocked_ValueTypeName_
{

    boolean template = ( this.getClass().getCanonicalName().contains( "_" ) );

    protected ColStorageBlocked_ValueTypeName_ store;

    @Before
    public void setup()
    {
        store = new ColStorageBlocked_ValueTypeName_( 1024, 1024 );
    }
    ...
    public void initTest()
    {
        ...
        store = new ColStorageBlocked_ValueTypeName_( 1024, 401 );
        ...
        store = new ColStorageBlocked_ValueTypeName_( 4096, 1024 ); //block size is 2^12
    ...
    public void fillTest()
    {
        ...
        {
            store.setValue( IntValueConverter._val_FromInt( i % 4 ), i );
        }
        //check all values
        for( int i = 0; i < 1024; i++ )
        {
            TestCase.assertEquals( IntValueConverter._val_FromInt( i % 4 ), store.getValue( i ) );
        }

        //insert same values in a different store with different block size
        ColStorageBlocked_ValueTypeName_ oneBlockStore = new ColStorageBlocked_ValueTypeName_( 1024, 1024 );  //one block
    ...
    public void overrideTest()
    {
        ...
        {
            store.setValue( IntValueConverter._val_FromInt( 10 ), i );
        }
        assertValues( IntValueConverter._val_FromInt( 10 ), 100, 300 );
    ...
    {
        if( template ) return;
        store = new ColStorageBlocked_ValueTypeName_( 2, 4 );
        try
        {
            store.setValue( IntValueConverter._val_FromInt( 3 ), 5 );
            TestCase.fail();
        }
        catch( Exception e )
        {
        }
        store.setValue( IntValueConverter._val_FromInt( 3 ), 3 );
    }
    ...
    {
        if( template ) return;
        ColStorageBlocked_ValueTypeName_ store2 = store.getCopy();
    ...
    {
        if( template ) return;
        store = new ColStorageBlocked_ValueTypeName_( 1024, 128 );
        store.fill( IntValueConverter._val_FromInt( 2 ) );
        store.fill( IntValueConverter._val_FromInt( 5 ), 10, 20 );

        assertValues( IntValueConverter._val_FromInt( 2 ), 0, 10 );
        assertValues( IntValueConverter._val_FromInt( 5 ), 10, 20 );
        assertValues( IntValueConverter._val_FromInt( 2 ), 20, 128 );
    }
    ...
    {
        if( template ) return;
        ColStorageBlocked_ValueTypeName_ source = new ColStorageBlocked_ValueTypeName_( 1024, 512 );
        source.fill( IntValueConverter._val_FromInt( 1 ) );

        source.fill( IntValueConverter._val_FromInt( 2 ), 100, 200 );
        source.fill( IntValueConverter._val_FromInt( 3 ), 250, 350 );
        source.fill( IntValueConverter._val_FromInt( 4 ), 350, 450 );

        ColStorageBlocked_ValueTypeName_ copyOfSource = source.getCopy();
        assertEquals( source, copyOfSource );
        copyOfSource.fill( IntValueConverter._val_FromInt( 6 ), 500, 512 );
    ...
    protected void assertValues( _val_ value, int startIdx, int endIdx )
    {
        int idxPtr = startIdx;
    ...
    protected void assertSame( ColStorageBlocked_ValueTypeName_ a, ColStorageBlocked_ValueTypeName_ b, int startIdx, int endIdx )
    {
    ...
    protected void assertEquals( ColStorageBlocked_ValueTypeName_ a, ColStorageBlocked_ValueTypeName_ b )
    {
