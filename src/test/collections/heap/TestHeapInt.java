...
public class TestHeapInt
{
    ...
    public void setup ()
    {
        heap = new HeapInt (8, new Comparators.IntAsc());
    }
    ...
    public void testInsertCompact ()
    {
        ...
        for (int i = 0; i < TEST_SIZE; i++)
        {
            TestCase.assertEquals (i,
                                   heap.insert (IntValueConverter.intFromInt (i)));
        }
        ...
        TestCase.assertEquals (3,
                               heap.insert (IntValueConverter.intFromInt (3)));
        TestCase.assertEquals (TEST_SIZE, heap.getSize ());
    ...
    public void testReverseInsertionCorrectOrder ()
    {
        ...
        {
            TestCase.assertEquals (i,
                                   heap.insert (IntValueConverter.intFromInt (TEST_SIZE - i)));
        }
    ...
    public void testRandomInsertionCorrectOrder ()
    {
        ...
        {
            TestCase.assertEquals (i,
                                   heap.insert (IntValueConverter.intFromInt (random.nextInt (16))));
        }
    }
    ...
    public void testBackwardsWithDoubleGrowth ()
    {
        ...
        {
            TestCase.assertEquals (i,
                                   heap.insert (IntValueConverter.intFromInt (24 - i)));
        }
    ...
    public void manyWithSameOneHigher ()
    {
        ...
        {
            TestCase.assertEquals (i,
                                   heap.insert (IntValueConverter.intFromInt (4)));
        }

        heap.insert (IntValueConverter.intFromInt (5));
        TestCase.assertEquals (IntValueConverter.intFromInt (4), heap.peek ());
        TestCase.assertEquals (25, heap.getSize ());
        for (int i = 0; i < 24; i++)
        {
            heap.removeTop ();
        }
        TestCase.assertEquals (IntValueConverter.intFromInt (5), heap.peek ());
    }
    ...
    public void manyWithSameOneLower ()
    {
        ...
        heap.insert (IntValueConverter.intFromInt (3));
        TestCase.assertEquals (IntValueConverter.intFromInt (3), heap.peek ());
    }
    ...
    protected void assertInAscOrder (HeapInt heap)
    {
        if (template) return;
        int[] collected = new int[heap.getSize ()];
        int ct = 0;
        while (!heap.isEmpty ())
        {
        ...
        int[] copy = new int[collected.length];
        System.arraycopy (collected, 0, copy, 0, collected.length);
        Arrays.sort (copy);
        TestUtilsInt.assertArrayContentsEqual (collected, copy);
    }
