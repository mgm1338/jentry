...
public class TestHeapShort
{
    ...
    public void setup ()
    {
        heap = new HeapShort (8, new Comparators.ShortAsc());
    }
    ...
    public void testInsertCompact ()
    {
        ...
        for (int i = 0; i < TEST_SIZE; i++)
        {
            TestCase.assertEquals (i,
                                   heap.insert (IntValueConverter.shortFromInt (i)));
        }
        ...
        TestCase.assertEquals (3,
                               heap.insert (IntValueConverter.shortFromInt (3)));
        TestCase.assertEquals (TEST_SIZE, heap.getSize ());
    ...
    public void testReverseInsertionCorrectOrder ()
    {
        ...
        {
            TestCase.assertEquals (i,
                                   heap.insert (IntValueConverter.shortFromInt (TEST_SIZE - i)));
        }
    ...
    public void testRandomInsertionCorrectOrder ()
    {
        ...
        {
            TestCase.assertEquals (i,
                                   heap.insert (IntValueConverter.shortFromInt (random.nextInt (16))));
        }
    }
    ...
    public void testBackwardsWithDoubleGrowth ()
    {
        ...
        {
            TestCase.assertEquals (i,
                                   heap.insert (IntValueConverter.shortFromInt (24 - i)));
        }
    ...
    public void manyWithSameOneHigher ()
    {
        ...
        {
            TestCase.assertEquals (i,
                                   heap.insert (IntValueConverter.shortFromInt (4)));
        }

        heap.insert (IntValueConverter.shortFromInt (5));
        TestCase.assertEquals (IntValueConverter.shortFromInt (4), heap.peek ());
        TestCase.assertEquals (25, heap.getSize ());
        for (int i = 0; i < 24; i++)
        {
            heap.removeTop ();
        }
        TestCase.assertEquals (IntValueConverter.shortFromInt (5), heap.peek ());
    }
    ...
    public void manyWithSameOneLower ()
    {
        ...
        heap.insert (IntValueConverter.shortFromInt (3));
        TestCase.assertEquals (IntValueConverter.shortFromInt (3), heap.peek ());
    }
    ...
    protected void assertInAscOrder (HeapShort heap)
    {
        if (template) return;
        short[] collected = new short[heap.getSize ()];
        int ct = 0;
        while (!heap.isEmpty ())
        {
        ...
        short[] copy = new short[collected.length];
        System.arraycopy (collected, 0, copy, 0, collected.length);
        Arrays.sort (copy);
        TestUtilsShort.assertArrayContentsEqual (collected, copy);
    }
