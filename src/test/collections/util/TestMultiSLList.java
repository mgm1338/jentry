...
public class TestMultiSLList
{
    ...
    /**
     * Tests that the references are not shared, the multilist is now nulled out,
     * but the copy has the same attributes
     */
    @Test
    public void copyThenNullSource()
    {
        MultiLinkedListInt copy = dataLoadedLists.getDeepCopy();
        dataLoadedLists = null;

        TestCase.assertEquals( copy.getSize(), 6 );
        assertListContents( copy, 0, 9, 8 );
        assertListContents( copy, 1, 17, 16, 15 );
        assertListContents( copy, 2, 6 );
    }

    /** Test that 1/2 way through inserting a bulk set, the state will end up being the same as */
    @Test
    public void insertHalfCopy()
    {

        MultiLinkedListInt newDataLoadedList = new MultiLinkedListInt( 3, 6 );
        newDataLoadedList.insert( 0, 9 );
        newDataLoadedList.insert( 1, 17 );
        newDataLoadedList.insert( 2, 6 );
        MultiLinkedListInt copy = newDataLoadedList.getDeepCopy();
        copy.insert( 0, 8 );
        copy.insert( 1, 16 );
        copy.insert( 1, 15 );
        TestCase.assertEquals( copy.getSize(), 6 );
        ...
        //asserting internals
        TestCase.assertEquals( dataLoadedLists.size, copy.size );
        TestCase.assertEquals( dataLoadedLists.freeListPtr, copy.freeListPtr );
        TestCase.assertEquals( dataLoadedLists.maxHead, copy.maxHead );
        TestCase.assertEquals( dataLoadedLists.nextUnusedIdx, copy.nextUnusedIdx );
        TestCase.assertEquals( dataLoadedLists.heads.length, copy.heads.length );

        TestUtilsInt.assertArrayContentsEqual( dataLoadedLists.heads, copy.heads );
        TestUtilsInt.assertArrayContentsEqual( dataLoadedLists.nexts, copy.nexts );
    }
