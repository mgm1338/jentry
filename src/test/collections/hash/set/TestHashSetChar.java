...
public class TestHashSetChar
{
    ...
    public void loadTest()
    {
        ...
        for( int i = 0; i < TEST_SIZE; i++ )
        {
            ...
            TestCase.assertTrue( hashSet.contains( IntValueConverter.charFromInt( i + OFFSET_FROM_ZERO ) ) == i );
        }
    ...
    public void removeFromSameBucket()
    {
        ...
        TestCase.assertEquals( Const.NO_ENTRY, hashSet.contains( IntValueConverter.charFromInt( 0 + OFFSET_FROM_ZERO
        ) ) );
        ...
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter.charFromInt( TEST_SIZE - 1 + OFFSET_FROM_ZERO ) )
                             == Const.NO_ENTRY );
        TestCase.assertEquals( Const.NO_ENTRY, hashSet.contains( IntValueConverter.charFromInt( TEST_SIZE - 1 +
                                                                                                 OFFSET_FROM_ZERO ) ) );
        ...
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter.charFromInt( TEST_SIZE / 2 + OFFSET_FROM_ZERO ) )
                             == Const.NO_ENTRY );
        TestCase.assertEquals( Const.NO_ENTRY, hashSet.contains( IntValueConverter.charFromInt( TEST_SIZE / 2 +
                                                                                                 OFFSET_FROM_ZERO
        ) ) );
    ...
    public void fullRemove()
    {
        ...
        for( int i = 0; i < TEST_SIZE; i++ )
        {
            TestCase.assertNotSame( hashSet.remove( IntValueConverter.charFromInt( i + OFFSET_FROM_ZERO ) ), Const.NO_ENTRY );
    ...
    public void assertRemoveIsNotThereFalse()
    {
        if( template ) return;
        fullRemove();
        TestCase.assertEquals( Const.NO_ENTRY, hashSet.remove( IntValueConverter.charFromInt( 1000 ) ) );
    ...
    public void freeListCompactNessTest()
    {
        ...
        TestCase.assertTrue( hashSet.contains( IntValueConverter.charFromInt( 1000 ) )==0 );
        TestCase.assertEquals( Const.NO_ENTRY, hashSet.contains( IntValueConverter.charFromInt( 0 ) ) );
    ...
    public void growFreeListTest()
    {
        ...
        for( int i = 0; i < TEST_SIZE * 4; i++ )
        {
            TestCase.assertNotSame( Const.NO_ENTRY, hashSet.remove( IntValueConverter.charFromInt( i + OFFSET_FROM_ZERO
            ) ) );
        }
        TestCase.assertTrue( hashSet.isEmpty() );
    ...
    public void clearTest()
    {
        ...
        for( int i = 0; i < TEST_SIZE; i++ )
        {
            TestCase.assertEquals( IntValueConverter.charFromInt( i + OFFSET_FROM_ZERO ), hashSet.get( i ) );
        }
        //clear
        hashSet.clear();
    ...
    public void copyToLargerSet()
    {
        if( template ) return;
        loadTest();
        HashSetChar copy = hashSet.copy( new HashSetChar( 4096 ) );
        assertEquals( hashSet, copy );
    }
    ...
    public void copyFromSmaller()
    {
        if( template ) return;
        loadTest();
        HashSetChar copy = hashSet.copy( new HashSetChar( 1 ) );
        assertEquals( hashSet, copy );
     ...
     */
    public static void assertEquals( HashSetChar expected, HashSetChar actual )
    {
        TestUtilsChar.assertArrayContentsToLen( expected.keys, actual.keys, expected.keys.length );
