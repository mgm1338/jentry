...
public class TestHashSetShort
{
    ...
    public void loadTest()
    {
        ...
        for( int i = 0; i < TEST_SIZE; i++ )
        {
            ...
            TestCase.assertTrue( hashSet.contains( IntValueConverter.shortFromInt( i ) ) );
        }
    ...
    public void removeFromSameBucket()
    {
        ...
        sameBucketTest();
        hashSet.remove( IntValueConverter.shortFromInt( 0 ) ); //remove first
        TestCase.assertEquals( hashSet.size, TEST_SIZE - 1 );
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter.shortFromInt( 0 ) ) == Const.NO_ENTRY );
        TestCase.assertFalse( hashSet.contains( IntValueConverter.shortFromInt( 0 ) ) );


        sameBucketTest(); //reprime
        hashSet.remove( IntValueConverter.shortFromInt( TEST_SIZE - 1 ) ); //remove last
        TestCase.assertEquals( hashSet.size, TEST_SIZE - 1 );
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter.shortFromInt( TEST_SIZE - 1 ) ) == Const.NO_ENTRY );
        TestCase.assertFalse( hashSet.contains( IntValueConverter.shortFromInt( TEST_SIZE - 1  ) ) );


        sameBucketTest(); //reprime
        hashSet.remove( IntValueConverter.shortFromInt( TEST_SIZE / 2 ) ); //remove middle
        TestCase.assertEquals( hashSet.size, TEST_SIZE - 1 );
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter.shortFromInt( TEST_SIZE / 2 ) ) == Const.NO_ENTRY );
        TestCase.assertFalse( hashSet.contains( IntValueConverter.shortFromInt( TEST_SIZE / 2  ) ) );
    ...
    public void fullRemove()
    {
        ...
        for( int i = 0; i < TEST_SIZE; i++ )
        {
            TestCase.assertTrue( hashSet.remove( IntValueConverter.shortFromInt( i ) ));
            TestCase.assertEquals( hashSet.getSize(), ( TEST_SIZE - i - 1 ) );
            TestCase.assertTrue( hashSet.getEntry( IntValueConverter.shortFromInt( i ) ) == Const.NO_ENTRY );
        }
        TestCase.assertTrue( hashSet.getSize() == 0 );
        TestCase.assertTrue( hashSet.isEmpty() );
