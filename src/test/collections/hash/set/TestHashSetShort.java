...
public class TestHashSetShort
{
    ...
    public void loadTest()
    {
        ...
        for( int i = 0; i < TEST_SIZE; i++ )
        {
            ...
            TestCase.assertTrue( hashSet.contains( IntValueConverter.shortFromInt( i + OFFSET_FROM_ZERO ) ) );
        }
        ...
        for( int i = 0; i < TEST_SIZE; i++ )
        {
            TestCase.assertEquals( hashSet.get( i ), IntValueConverter.shortFromInt( i + OFFSET_FROM_ZERO ) );
        }
    }
    ...
    public void removeFromSameBucket()
    {
        ...
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter.shortFromInt( 0 + OFFSET_FROM_ZERO ) ) == Const.NO_ENTRY );
        TestCase.assertFalse( hashSet.contains( IntValueConverter.shortFromInt( 0 + OFFSET_FROM_ZERO ) ) );


        sameBucketTest(); //reprime
        hashSet.remove( IntValueConverter.shortFromInt( TEST_SIZE - 1 + OFFSET_FROM_ZERO ) ); //remove last
        TestCase.assertEquals( TEST_SIZE - 1, hashSet.size );
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter.shortFromInt( TEST_SIZE - 1 + OFFSET_FROM_ZERO ) )
                             == Const.NO_ENTRY );
        TestCase.assertFalse( hashSet.contains( IntValueConverter.shortFromInt( TEST_SIZE - 1 + OFFSET_FROM_ZERO ) ) );


        sameBucketTest(); //reprime
        hashSet.remove( IntValueConverter.shortFromInt( TEST_SIZE / 2 + OFFSET_FROM_ZERO ) ); //remove middle
        TestCase.assertEquals( TEST_SIZE - 1, hashSet.size );
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter.shortFromInt( TEST_SIZE / 2 + OFFSET_FROM_ZERO ) )
                             == Const.NO_ENTRY );
        TestCase.assertFalse( hashSet.contains( IntValueConverter.shortFromInt( TEST_SIZE / 2 + OFFSET_FROM_ZERO ) ) );
    ...
    public void fullRemove()
    {
        ...
        for( int i = 0; i < TEST_SIZE; i++ )
        {
            TestCase.assertNotSame( hashSet.remove( IntValueConverter.shortFromInt( i + OFFSET_FROM_ZERO ) ), Const.NO_ENTRY );
            TestCase.assertEquals( ( TEST_SIZE - i - 1 ), hashSet.getSize() );
            TestCase.assertTrue( hashSet.getEntry( IntValueConverter.shortFromInt( i + OFFSET_FROM_ZERO ) )
                                 == Const.NO_ENTRY );
        }
        TestCase.assertTrue( hashSet.getSize() == 0 );
        TestCase.assertTrue( hashSet.isEmpty() );
    ...
    public void assertRemoveIsNotThereFalse()
    {
        if( template ) return;
        fullRemove();
        TestCase.assertEquals( Const.NO_ENTRY, hashSet.remove( IntValueConverter.shortFromInt( 1000 ) ) );
    ...
    public void freeListCompactNessTest()
    {
        ...
        TestCase.assertTrue( hashSet.contains( IntValueConverter.shortFromInt( 1000 ) ) );
        TestCase.assertFalse( hashSet.contains( IntValueConverter.shortFromInt( 0 ) ) );
    ...
    public void growFreeListTest()
    {
        ...
        for( int i = 0; i < TEST_SIZE * 4; i++ )
        {
            TestCase.assertNotSame( Const.NO_ENTRY, hashSet.remove( IntValueConverter.shortFromInt( i + OFFSET_FROM_ZERO
            ) ) );
        }
        TestCase.assertTrue( hashSet.isEmpty() );
    ...
    public void clearTest()
    {
        ...
        for( int i = 0; i < TEST_SIZE; i++ )
        {
            TestCase.assertEquals( IntValueConverter.shortFromInt( i + OFFSET_FROM_ZERO ), hashSet.get( i ) );
        }
        //clear
        hashSet.clear();
    ...
    public void fullCopyValidSetTest()
    {
        if( template ) return;
        loadTest();
        HashSetShort copy = hashSet.copy( null );
    ...
    public void copyToLargerSet()
    {
        if( template ) return;
        loadTest();
        HashSetShort copy = hashSet.copy( new HashSetShort( 4096 ) );
        assertEquals( hashSet, copy );
    }
    ...
    public void copyFromSmaller()
    {
        if( template ) return;
        loadTest();
        HashSetShort copy = hashSet.copy( new HashSetShort( 1 ) );
        assertEquals( hashSet, copy );
     ...
     */
    public static void assertEquals( HashSetShort expected, HashSetShort actual )
    {
        TestUtilsShort.assertArrayContentsToLen( expected.keys, actual.keys, expected.keys.length );
