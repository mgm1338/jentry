...
public class TestHashSetObject
{
    ...
    public void loadTest()
    {
        ...
        {
            int j = hashSet.insert( IntValueConverter.ObjectFromInt( i + OFFSET_FROM_ZERO ) );
            TestCase.assertEquals( i, j ); //compact
            TestCase.assertTrue( hashSet.contains( IntValueConverter.ObjectFromInt( i + OFFSET_FROM_ZERO ) ) == i );
        }
        ...
        for( int i = 0; i < TEST_SIZE; i++ )
        {
            TestCase.assertEquals( hashSet.get( i ), IntValueConverter.ObjectFromInt( i + OFFSET_FROM_ZERO ) );
        }
    }
    ...
    public void removeFromSameBucket()
    {
        ...
        sameBucketTest();
        hashSet.remove( IntValueConverter.ObjectFromInt( 0 + OFFSET_FROM_ZERO ) ); //remove first
        TestCase.assertEquals( TEST_SIZE - 1, hashSet.size );
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter.ObjectFromInt( 0 + OFFSET_FROM_ZERO ) ) == Const.NO_ENTRY );
        TestCase.assertEquals( Const.NO_ENTRY, hashSet.contains( IntValueConverter.ObjectFromInt( 0 + OFFSET_FROM_ZERO
        ) ) );


        sameBucketTest(); //reprime
        hashSet.remove( IntValueConverter.ObjectFromInt( TEST_SIZE - 1 + OFFSET_FROM_ZERO ) ); //remove last
        TestCase.assertEquals( TEST_SIZE - 1, hashSet.size );
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter.ObjectFromInt( TEST_SIZE - 1 + OFFSET_FROM_ZERO ) )
                             == Const.NO_ENTRY );
        TestCase.assertEquals( Const.NO_ENTRY, hashSet.contains( IntValueConverter.ObjectFromInt( TEST_SIZE - 1 +
                                                                                                 OFFSET_FROM_ZERO ) ) );


        sameBucketTest(); //reprime
        hashSet.remove( IntValueConverter.ObjectFromInt( TEST_SIZE / 2 + OFFSET_FROM_ZERO ) ); //remove middle
        TestCase.assertEquals( TEST_SIZE - 1, hashSet.size );
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter.ObjectFromInt( TEST_SIZE / 2 + OFFSET_FROM_ZERO ) )
                             == Const.NO_ENTRY );
        TestCase.assertEquals( Const.NO_ENTRY, hashSet.contains( IntValueConverter.ObjectFromInt( TEST_SIZE / 2 +
                                                                                                 OFFSET_FROM_ZERO
        ) ) );
    ...
    public void freeListCompactNessTest()
    {
        ...
        TestCase.assertTrue( hashSet.contains( IntValueConverter.ObjectFromInt( 1000 ) )==0 );
        TestCase.assertEquals( Const.NO_ENTRY, hashSet.contains( IntValueConverter.ObjectFromInt( 0 ) ) );
    ...
    public void clearTest()
    {
        ...
        for( int i = 0; i < TEST_SIZE; i++ )
        {
            TestCase.assertEquals( IntValueConverter.ObjectFromInt( i + OFFSET_FROM_ZERO ), hashSet.get( i ) );
        }
        //clear
        hashSet.clear();
    ...
    public void fullCopyValidSetTest()
    {
        if( template ) return;
        loadTest();
        HashSetObject copy = hashSet.copy( null );
    ...
    public void copyToLargerSet()
    {
        if( template ) return;
        loadTest();
        HashSetObject copy = hashSet.copy( new HashSetObject( 4096 ) );
        assertEquals( hashSet, copy );
    }
    ...
    public void copyFromSmaller()
    {
        if( template ) return;
        loadTest();
        HashSetObject copy = hashSet.copy( new HashSetObject( 1 ) );
        assertEquals( hashSet, copy );
     ...
     */
    public static void assertEquals( HashSetObject expected, HashSetObject actual )
    {
        TestUtilsObject.assertArrayContentsToLen( expected.keys, actual.keys, expected.keys.length );
