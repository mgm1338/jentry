...
public class TestHashSet_KeyTypeName_
{
    ...
    public void removeFromSameBucket()
    {
        ...
        TestCase.assertEquals( Const.NO_ENTRY, hashSet.contains( IntValueConverter._key_FromInt( 0 + OFFSET_FROM_ZERO
        ) ) );
        ...
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter._key_FromInt( TEST_SIZE - 1 + OFFSET_FROM_ZERO ) )
                             == Const.NO_ENTRY );
        TestCase.assertEquals( Const.NO_ENTRY, hashSet.contains( IntValueConverter._key_FromInt( TEST_SIZE - 1 +
                                                                                                 OFFSET_FROM_ZERO ) ) );
        ...
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter._key_FromInt( TEST_SIZE / 2 + OFFSET_FROM_ZERO ) )
                             == Const.NO_ENTRY );
        TestCase.assertEquals( Const.NO_ENTRY, hashSet.contains( IntValueConverter._key_FromInt( TEST_SIZE / 2 +
                                                                                                 OFFSET_FROM_ZERO
        ) ) );
    ...
    public void assertRemoveIsNotThereFalse()
    {
        if( template ) return;
        fullRemove();
        TestCase.assertEquals( Const.NO_ENTRY, hashSet.remove( IntValueConverter._key_FromInt( 1000 ) ) );
    ...
    public void growFreeListTest()
    {
        ...
        for( int i = 0; i < TEST_SIZE * 4; i++ )
        {
            TestCase.assertNotSame( Const.NO_ENTRY, hashSet.remove( IntValueConverter._key_FromInt( i + OFFSET_FROM_ZERO
            ) ) );
        }
        TestCase.assertTrue( hashSet.isEmpty() );
    ...
    public void clearTest()
    {
        ...
        for( int i = 0; i < TEST_SIZE; i++ )
        {
            TestCase.assertEquals( IntValueConverter._key_FromInt( i + OFFSET_FROM_ZERO ), hashSet.get( i ) );
        }
        //clear
        hashSet.clear();
     ...
     */
    public static void assertEquals( HashSet_KeyTypeName_ expected, HashSet_KeyTypeName_ actual )
    {
