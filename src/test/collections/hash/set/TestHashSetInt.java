...
import collections.hash.set.HashSetInt;
...
public class TestHashSetInt
{
    ...
    {
        if( template ) return;
        hashSet = new HashSetInt( TEST_SIZE );
        ...
        {
            int j = hashSet.insert( IntValueConverter.intFromInt( i ) );
            TestCase.assertEquals( i, j ); //compact
            TestCase.assertTrue( hashSet.contains( IntValueConverter.intFromInt( i ) ) );
        }
        ...
        for( int i = 0; i < TEST_SIZE; i++ )
        {
            TestCase.assertEquals( hashSet.get( i ), IntValueConverter.intFromInt( i ) );
        }
    }
    ...
    public void sameBucketTest()
    {
        ...
        hashSet = new HashSetInt( 8, 1.00, ArrayFactoryInt.defaultIntProvider,
                                            ArrayFactoryInt.defaultIntProvider, new SameBucketHashFunctionInt(),
    ...
    public void removeFromSameBucket()
    {
        ...
        sameBucketTest();
        hashSet.remove( IntValueConverter.intFromInt( 0 ) ); //remove first
        TestCase.assertEquals( hashSet.size, TEST_SIZE - 1 );
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter.intFromInt( 0 ) ) == Const.NO_ENTRY );
        TestCase.assertFalse( hashSet.contains( IntValueConverter.intFromInt( 0 ) ) );


        sameBucketTest(); //reprime
        hashSet.remove( IntValueConverter.intFromInt( TEST_SIZE - 1 ) ); //remove last
        TestCase.assertEquals( hashSet.size, TEST_SIZE - 1 );
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter.intFromInt( TEST_SIZE - 1 ) ) == Const.NO_ENTRY );
        TestCase.assertFalse( hashSet.contains( IntValueConverter.intFromInt( TEST_SIZE - 1  ) ) );


        sameBucketTest(); //reprime
        hashSet.remove( IntValueConverter.intFromInt( TEST_SIZE / 2 ) ); //remove middle
        TestCase.assertEquals( hashSet.size, TEST_SIZE - 1 );
        TestCase.assertTrue( hashSet.getEntry( IntValueConverter.intFromInt( TEST_SIZE / 2 ) ) == Const.NO_ENTRY );
        TestCase.assertFalse( hashSet.contains( IntValueConverter.intFromInt( TEST_SIZE / 2  ) ) );
    ...
    public void fullRemove()
    {
        ...
        for( int i = 0; i < TEST_SIZE; i++ )
        {
            TestCase.assertTrue( hashSet.remove( IntValueConverter.intFromInt( i ) ));
            TestCase.assertEquals( hashSet.getSize(), ( TEST_SIZE - i - 1 ) );
            TestCase.assertTrue( hashSet.getEntry( IntValueConverter.intFromInt( i ) ) == Const.NO_ENTRY );
        }
        TestCase.assertTrue( hashSet.getSize() == 0 );
        TestCase.assertTrue( hashSet.isEmpty() );
    ...
    public void assertRemoveIsNotThereFalse()
    {
        if (template) return;
        fullRemove();
        TestCase.assertFalse( hashSet.remove( IntValueConverter.intFromInt( 1000 ) ) );
    ...
    protected class SameBucketHashFunctionInt extends HashFunctions.HashFunctionInt
    {
        @Override
